...
import gbmHttp from './gbm'

describe('gbm request client', () => {
  beforeEach(() => {
    localStorage.clear()
  ...
  it('injects bearer token from user_auth_info storage', async () => {
    ...
    const interceptor = gbmHttp.interceptors.request.handlers?.[0]?.fulfilled
    ...
    expect(config.headers.get('Authorization')).toBe('Bearer gbm-token')
  ...
  it('clears auth storage and redirects to /login on 401', async () => {
    ...
    const interceptor = gbmHttp.interceptors.response.handlers?.[0]?.rejected
    expect(interceptor).toBeTypeOf('function')
